...
// Convierte los archivos SCSS importantes a JS para el libro:
...
const generateHTML = async () => {
  ...
  <center style="page-break-after: always">
    ...
    <p style='width: 340px;'>Esta copia del manual de TypeScript se creó el ${date} contra
    la confirmación
...
const generatePDF = async () => {
  ...
  const browser = await chromium.launch(); // 'firefox' o 'webkit'.
...
const tableOfContents = (items: any[]) => {
  const start = `<h1 style='margin: 0 2rem; margin-bottom: 4rem;'>Tabla de contenido</h1><table><tbody>`;
