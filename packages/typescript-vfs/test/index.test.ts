...
// Anteriormente, lib.dom.d.ts no estaba incluido
...
// Asegura que las personas puedan incluir algo como lib es2015, etc.
...
it("creates a map from the CDN without cache", async () => {
  ...
  fetcher.mockResolvedValue({ text: () => Promise.resolve("// Contenido del archivo") })
...
it("creates a map from the CDN and uses the existing local storage cache", async () => {
  ...
  // Una vez devuelto un valor del almacén
  store.getItem.mockReturnValueOnce("// Desde caché")
  ...
  // Debería ser una recuperación menos, y en su lugar, el primer elemento sería de la caché
  expect(store.setItem).toBeCalledTimes(libs.length - 1)
  expect(map.get("/" + libs[0])).toMatchInlineSnapshot(`"// Desde caché"`)
})
