...
console.log("TSConfig Ref: MD para TSConfig");
...
// Se asegura de que todas las categorías se tengan en cuenta en ^
...
// Extrae todo lo que no vive en compilerOptions
...
// La referencia de TSConfig es una colección de secciones que tienen opciones o
// una colección de categorías que tienen opciones
const sections = [
...
languages.forEach((lang) => {
  ...
  const getPathInLocale = (path: string, optionalExampleContent?: string, failable = false) => {
    ...
    if (!failable)
      // prettier-ignore
      throw new Error("No puedo encontrar una ruta para " + path + " in " + localeDesc + " " + optionalExampleContent || "");
  };

  // Hace un volcado JSON de la categoría que ancla a algo que envuelve el markdown
  ...
  sections.forEach((section) => {
    ...
    // Muestra una subnavegación fija para las categorías
    ...
    sectionCategories.forEach((categoryID) => {
      // Necesitamos esto para buscar el ID de categoría
      ...
      if (category) {
        ...
        assert.ok(categoryFile.data.display, "No display data for category: " + categoryID); // Debe tener un título de visualización en el tema principal
        categoryName = categoryFile.data.display;

        mdChunks.push("<div class='category'>");

        // Deja que el título cambie su pantalla pero que mantenga el mismo ID
        ...
        // Empuja la copia de category
        mdChunks.push(categoryFile.content);
        mdChunks.push("</div>");
      }

      // Extrae sus opciones para la sección.
      ...
      optionsForCategory.forEach((option) => {
        ...
        // Debe tener un título de visualización en el tema principal
        // prettier-ignore
        ...
        // Hace un contenedor flexbox para la tabla y el contenido.
        mdChunks.push("<div class='compiler-content'>");

        mdChunks.push("<div class='markdown'>");
        // Empuja el contenido del archivo
        mdChunks.push(optionFile.content);

        mdChunks.push("</div>");

        // Hace una tabla markdown de los metadatos importantes
        ...
        if (option.related) {
          const optionValue = option.related.map(
            (r) =>
              `<a href='#${r}' aria-label="Salta a la información de la opción del compilador para ${r}" ><code>${r}</code></a>`
          );
        ...
        if (option.releaseVersion) {
          ...
          mdTableRows.push([
            "Liberado",
            `<a aria-label="Notas de la versión para TypeScript ${option.releaseVersion}" href="${link}">${option.releaseVersion}</a>`,
          ]);
        }
  ...
  // Esto lo utilizan las ventanas emergentes tsconfig
  writeFileSync(
...
// Desde https://stackoverflow.com/questions/8495687/split-array-into-chunks
