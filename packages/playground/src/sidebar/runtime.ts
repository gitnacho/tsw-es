...
// Gracias SO: https://stackoverflow.com/questions/20256760/javascript-console-log-to-html/35449256#35449256
...
function rewireLoggingToElement(
  ...
  closure.then(js => {
    ...
    try {
      ...
      if (error instanceof SyntaxError && /\bexport\b/u.test(error.message)) {
        console.warn(
          'Consejo: Cambia la configuración del Módulo a "CommonJS" en la configuración de TS Config para permitir que las exportaciones de nivel superior funcionen en Playground.'
        )
      }
    }
  })
  ...
  // Constantes en línea que se cambian al final del procesamiento
  const replacers = {
  ...
  const objectToText = (arg: any): string => {
    ...
    if (arg && arg.stack && arg.message) {
      ...
      // Nadie necesita saber que un obj es un obj
      const nameWithoutObject = name && name === "Object" ? "" : htmlEscape(name)
      const prefix = nameWithoutObject ? `${nameWithoutObject}: ` : ""

      // JSON.stringify omite cualquier clave con un valor indefinido. Para evitar esto, reemplazamos undefined con el texto __undefined__ y luego hacemos un reemplazo global usando expresiones regulares de vuelta a la palabra clave undefined
      textRep =
        prefix +
...
// El entorno de ejecución de reflect-metadata está disponible, así que permite que pases
