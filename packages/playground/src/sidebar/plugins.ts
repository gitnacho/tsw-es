...
/** Si el playground se debería comunicar activamente con un complemento existente */
...
export const optionsPlugin: PluginFactory = (i, utils) => {
  const plugin: PlaygroundPlugin = {
    ...
    // shouldBeSelected: () => true, // elimina el comentario para que esta sea la primera pestaña en recargas
  ...
  const createPlugin = (plugin: typeof allNPMPlugins[0]) => {
    ...
    // Evita XSS por parte de alguien que inyecte JS a través de la descripción, que es el único texto libre que alguien puede usar
