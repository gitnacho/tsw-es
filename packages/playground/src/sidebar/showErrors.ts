...
export const showErrors: PluginFactory = (i, utils) => {
  ...
  const updateUI = () => {
    ...
    // Fianza anticipada si no hay nada que mostrar
    if (!markers.length) {
      prevMarkers = []
    ...
    // El pasar por encima el mouse puede desencadenar esto, así que evita ese bucle.
    ...
    // Limpia cualquier posible pantalla vacía
    ds.clear()
