...
// De aquí es de donde provienen todas las descripciones localizadas
...
export const createConfigDropdown = (sandbox: Sandbox, monaco: Monaco) => {
  ...
  // Queremos hacer secciones de categorías
  ...
  boolOptions.forEach(opt => {
    ...
    // Cuando se agrega una nueva bandera del compilador, y aún no está en el sitio, esta búsqueda fallará.
    // Luego vuelve a crear uno a partir de la información en el compilador
    if (!summary) {
      summary = {
        display: opt.name,
  ...
  // Al cambiar entre un archivo .ts y un archivo .tsx - actualiza el playground
...
const createSelect = (title: string, id: string, blurb: string, sandbox: Sandbox, option: any) => {
  ...
  select.onchange = () => {
    const value = select.value // la cadena human
