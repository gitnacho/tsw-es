...
// El operador de fusión nula es una alternativa a || que devuelve la
// expresión del lado derecho si el lado izquierdo es null o
// undefined.

// Por el contrario, || usa comprobaciones falsas, es decir, una
// cadena vacía o el número 0 se consideraría false.

// Un buen ejemplo de esta característica es el manejo parcial de
// objetos que tienen valores predeterminados cuando no se pasa una
// clave.
...
function updateApp(config: Partial<AppConfiguration>) {
  ...
  config.name = config.name || "(sin nombre)"; // no permite la
																							 // entrada ""
  config.items = config.items || -1; // no permite la entrada 0
  config.active = config.active || true; // realmente mal, siempre
																				 // true
}

// Puedes leer más sobre la fusión nula en el artículo del blog 3.7:
//
// https://devblogs.microsoft.com/typescript/announcing-typescript-3-7/
`

it("it to convert to markdown", () => {
  const markdown = invertCodeToHTML(code)
